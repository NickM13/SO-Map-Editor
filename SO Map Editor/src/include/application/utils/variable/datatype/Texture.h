...
public:
	...
	void render(Vector2<Sint32> p_start = {0, 0}, Vector2<Sint32> p_size = {-1, -1}) {
		...
		glBegin(GL_QUADS);
		{
			glTexCoord2f(tPos.x, tPos.y - tSize.y);						glVertex2f(0, 0);
			glTexCoord2f(tPos.x + tSize.x, tPos.y - tSize.y);				glVertex2f(p_size.x, 0);
			glTexCoord2f(tPos.x + tSize.x, tPos.y);	glVertex2f(p_size.x, p_size.y);
			glTexCoord2f(tPos.x, tPos.y);				glVertex2f(0, p_size.y);
		}
		glEnd();
	}
};
